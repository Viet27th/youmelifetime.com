<template>
    <div class="row">
        <div class="col-12">
            ...
            <div><strong class="text-danger d-block" v-for="(error,index) in notification_error_blog_create"
                         v-if="index === 0">{{error}}</strong></div>
                ...
                <div class="col-3">
                    <div class="card mb-3">
                        ...
                        <div class="card-footer clearfix">
                            <button type="button" class="btn btn-danger float-left btn-sm"
                                    @click="blogSoftDelete(blogId)">Soft Delete
                            </button>
                            <button type="button" class="btn btn-primary float-right btn-sm"
                                    @click="blogUpdate">Update
                            </button>
                        </div>
                    </div>
                </div>
  ...
  export default {
    ...
    mounted: function () {
      this.$nextTick(function () {
        //set initialize data
        let blog_info = JSON.parse(this.blogFirstData)
        let first_categories = JSON.parse(this.firstCategories)
        let first_tags = JSON.parse(this.firstTags)

        this.title = blog_info.title
        this.slug = blog_info.slug
        this.content = blog_info.content
        this.description = blog_info.description
        this.option_date = blog_info.option_date
        this.status = blog_info.status
        this.categories = first_categories
        this.tags = first_tags
        this.feature_image = blog_info.feature_image
        this.feature_image_id = blog_info.feature_image_id
        this.feature_image_alt = blog_info.feature_image_alt
        this.feature_image_title = blog_info.feature_image_title
        this.option_date = blog_info.option_date
    ...
    methods: {
      ...
      //=========
      blogSoftDelete: function (blogId) {
        window.notification({
          data: {
            message: 'Soft Delete?',
            actionOk: () => {
              axios({
                ...
                .then(responsive => {
                  window.location = blogRoute.admin_blog
                })
                .catch(error => {
                  console.log(error)
                })
      ...
      blogUpdate: function () {
        axios({
          method: 'put',
          url: blogRoute.admin_blog_update + this.blogId,
          data: {
            title: this.title,
            slug: this.slug,
